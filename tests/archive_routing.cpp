...
TEST(BaseOutputArchiveRouting, SerializeMethodHandled)
{
    ...
    {
        NotSerializable ns;
        EXPECT_THROW_MESSAGE({ a << ns; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
        EXPECT_THROW_MESSAGE({ a & ns; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
    }
    ...
    {
        PrivateNoFriends pa;
        EXPECT_THROW_MESSAGE({ a << pa; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
        EXPECT_THROW_MESSAGE({ a & pa; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
    }

    {
        ArchiveValue av;
        EXPECT_THROW_MESSAGE({ a << av; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
        EXPECT_THROW_MESSAGE({ a & av; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
    }
}
...
TEST(BaseOutputArchiveRouting, SaveMethodHandled)
{
    ...
    {
        NonConstSavable ncs;
        EXPECT_THROW_MESSAGE({ a << ncs; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
        EXPECT_THROW_MESSAGE({ a & ncs; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
    }
...

    {
        ...
        EXPECT_THROW_MESSAGE({ a << pnf; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
        EXPECT_THROW_MESSAGE({ a & pnf; }, KTSERIAL_ERROR_NO_HANDLERS_FOUND);
    }
